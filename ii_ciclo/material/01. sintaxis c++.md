...
### Función para leer enteros sin error
...
Para eliminar un elemento se utiliza `erase()`.

```cpp
peliculas.erase(peliculas.begin() + posicion);
```

Si `posicion` vale `2`, se elimina el tercer elemento del vector.

`begin()` representa el primer elemento del vector y se utiliza junto con `erase()` para indicar qué posición se desea eliminar.
...
- `true` si el vector no contiene elementos;
