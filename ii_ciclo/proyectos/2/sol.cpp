...
void cargarDesdeArchivo() {
    ...
    if (!archivo.is_open()) {
        cout << "Archivo peliculas.csv no encontrado. Se iniciara con catalogo vacio.\n";
        return;
    }
    ...
    while (getline(archivo, linea)) {
        ...
        string codigoTexto, nombre, genero, duracionTexto, calificacionTexto;
        ...
        // Intentar convertir; si falla, se ignora la línea
        try {
            Pelicula p;
            p.codigo = stoi(codigoTexto);
            p.nombre = nombre;
            p.genero = genero;
            p.duracion = stoi(duracionTexto);
            p.calificacion = stod(calificacionTexto);

            // Validar que los datos sean correctos
            if (p.codigo > 0 && p.duracion > 0 && p.calificacion >= 0 && p.calificacion <= 10) {
    ...
    cout << "Archivo peliculas.csv cargado correctamente (" << contador << " peliculas).\n";
}

void guardarCatalogo() {
    ofstream archivo("peliculas.csv");
    ...
    cout << "\nCatalogo guardado correctamente en peliculas.csv.\n";
}